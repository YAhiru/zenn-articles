...
リファクタリングと一口に言っても様々な性格のものが存在しますが, その中でも**ある一定のルールに基づく機械的な作業**が要求されるものがあるかと思います。
...
本記事では予め用意された[サンプルコード](https://github.com/YAhiru/rector-tutorial)に対して Rector を実際に実行することで理解を深めていきたいと思います。
...
今回はあなたの所属するチーム内で「PHPUnit のテストメソッドは, メソッド名に `test` という prefix を付ける形式ではなく `@test` アノテーションを使う形式にしたい」という要望が出たという設定で進めていきましょう。^[あくまで記事の都合上の話であり @test アノテーションを推奨しているわけではありません。]
...
実は本記事で作成したルールは, 現状の実装のままでは**あまり好ましくない挙動をするケース**があります。
